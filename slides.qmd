...
- An environment is defined by a file with metadata on
  - Which packages, versions, from where
...
- Many tools to interact with PyPI and they achieve similar things
- `pip` + `venv` is available by default for Python 3.3+
...
- Have the environment inside your project directory (default in R)
- When changing/updating dependencies, commit often and share with your coauthors
- While project is in progress, do not pin versions if not necessary 
...
- If you're using `venv` or `conda`, try the other in a dummy project
